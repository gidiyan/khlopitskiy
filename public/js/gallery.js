...
function gallery() {
    return {
        ...
        init: function (t) {
            var e = this;
            if (void 0 !== t) for (var i = 0, r = Object.entries(t); i < r.length; i++) {
                var a = _slicedToArray(r[i], 2), l = a[0], s = a[1];
                this.default[l] = s
            }
            var n = this, h = this.$el, o = null, u = "";
            ["height", "width", "size"].forEach((function (t) {
                void 0 !== n.$refs[t] && (o = n.$refs[t].querySelector("img"), u = t)
            })), this.$el.querySelectorAll("." + this.default.item).forEach((function (t) {
                e.elements.push(t);
                var i = t.querySelector("img");
                if (null === i) return !1;
                var r = !1;
                i.classList.contains(e.default.lazy) && void 0 !== i.dataset.src && (r = !0);
                var a = new Image;
                a.onload = function () {
                    r && (i.classList.remove(n.default.lazy), i.src = a.src);
                    var t = a.naturalWidth / a.naturalHeight;
                    null !== o && o.src === i.src && ("height" === u ? h.style.setProperty("--gallery_height", a.height + "px") : "width" === u ? (h.style.setProperty("--gallery_width", a.width + "px"), h.style.setProperty("--gallery_height", Math.floor(i.width / t) + "px")) : "size" === u && (h.style.setProperty("--gallery_width", a.width + "px"), h.style.setProperty("--gallery_height", a.height + "px")))
                }, a.src = r ? i.dataset.src : i.src
            })), this.elements[0].classList.remove(this.default.hidden)
        },
        next: function () {
            this.last = this.active, this.elements.length > this.active + 1 ? this.active++ : this.active = 0;
            this.default.left_out, this.default.left_in;
            this.elements[this.active].classList.remove(this.default.left_out, this.default.left_in, this.default.right_out, this.default.hidden), this.elements[this.active].classList.add(this.default.right_in), this.elements[this.last].classList.add(this.default.right_out, this.default.hidden), this.elements[this.last].classList.remove(this.default.left_out, this.default.left_in, this.default.right_in)
        },
        previous: function () {
            this.last = this.active, this.active - 1 >= 0 ? this.active-- : this.active = this.elements.length - 1, this.elements[this.active].classList.remove(this.default.right_in, this.default.right_out, this.default.left_out, this.default.hidden), this.elements[this.active].classList.add(this.default.left_in), this.elements[this.last].classList.add(this.default.hidden, this.default.left_out), this.elements[this.last].classList.remove(this.default.right_in, this.default.right_out, this.default.left_in)
        }
    }
}
